...
"""
Training script for ResNet-based weather classification.
Builds and trains deep learning models (ResNet, AttResNet with coordinate attention,
CNN) based on NetCDF data (500hPa geopotential height z500 and 2m temperature anomaly
t2m-t2man). Supports summer (s)/winter (w) scenarios with grid search across days and
training/validation ratios.

Usage: python 04_train_resnet.py <s|w> <start> <end>
    s: summer, w: winter
    start/end: training iteration range [start, end)
"""
        ...
        if self.withT2m:
            self.data = np.concatenate(
                (self.z500[:, np.newaxis, self.day, :],
                 self.t2man[:, np.newaxis, self.day, :]), axis=1
            )
            input_dim = 2
        else:
            input_dim = 1
...
class resnet(nn.Module):
    def __init__(self):
        ...
        h = input_h
        w = input_w
        self.block1 = BasicBlock(input_dim, 64)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2)
        h = h // 2
        w = w // 2
        self.block2 = BasicBlock(64, 128)

        self.fc1 = nn.Linear(h * w * 128, num_classes)
...
class attresnet(nn.Module):
    def __init__(self):
        ...
        h = input_h
        w = input_w
        self.block1 = BasicBlock(input_dim, 128)
        self.CA_Block1 = CoordAtt(128, 128)

        self.fc1 = nn.Linear(h * w * 128, 50)
        self.fc2 = nn.Linear(50, num_classes)

    def forward(self, x):
        out = self.CA_Block1(self.block1(x))
